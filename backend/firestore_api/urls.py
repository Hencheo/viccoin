...
# Adicionar endpoints específicos de fallback
@api_view(['GET'])
@permission_classes([AllowAny])
def categorias_fallback(request):
    """
    Endpoint de fallback para categorias, caso o Firestore falhe
    """
    try:
        # Primeiro tenta usar o serviço real
        service = CategoriaService()
        categorias = service.get_all()
        if categorias:
            # Se conseguir dados do Firestore, retorna eles
            return Response([c.to_dict() for c in categorias])
        else:
            # Se não houver dados, usa os dados estáticos
            raise Exception("Sem dados no Firestore")
    except Exception as e:
        # Em caso de erro, usa dados estáticos
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Erro ao acessar categorias do Firestore: {str(e)}")
        
        # Dados estáticos para teste
        categorias = [
            {"id": "cat1", "nome": "Alimentação", "icone": "food", "cor": "#FF5733"},
            {"id": "cat2", "nome": "Transporte", "icone": "car", "cor": "#33FF57"},
            {"id": "cat3", "nome": "Moradia", "icone": "home", "cor": "#3357FF"},
            {"id": "cat4", "nome": "Lazer", "icone": "fun", "cor": "#F033FF"},
            {"id": "cat5", "nome": "Saúde", "icone": "health", "cor": "#33FFF0"}
        ]
        
        return Response({
            'status': 'ok',
            'message': 'Categorias carregadas com sucesso (dados estáticos)',
...
@api_view(['GET'])
@permission_classes([AllowAny])
def receitas_fallback(request):
    """
    Endpoint de fallback para receitas, caso o Firestore falhe
    """
    try:
        # Tenta usar o serviço real com ID de usuário de teste
        user_id = "user_teste_123"
        service = ReceitaService()
        receitas = service.get_all_by_user(user_id)
        if receitas:
            # Se conseguir dados do Firestore, retorna eles
            return Response([r.to_dict() for r in receitas])
        else:
            # Se não houver dados, usa os dados estáticos
            raise Exception("Sem dados no Firestore")
    except Exception as e:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Erro ao acessar receitas do Firestore: {str(e)}")
        ...
        return Response({
            'status': 'ok',
            'message': 'Receitas carregadas com sucesso (dados estáticos)',
            'receitas': receitas,
            'erro': str(e)
        })
...
@api_view(['GET'])
@permission_classes([AllowAny])
def despesas_fallback(request):
    """
    Endpoint de fallback para despesas, caso o Firestore falhe
    """
    try:
        # Tenta usar o serviço real com ID de usuário de teste
        user_id = "user_teste_123"
        service = DespesaService()
        despesas = service.get_all_by_user(user_id)
        if despesas:
            # Se conseguir dados do Firestore, retorna eles
            return Response([d.to_dict() for d in despesas])
        else:
            # Se não houver dados, usa os dados estáticos
            raise Exception("Sem dados no Firestore")
    except Exception as e:
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Erro ao acessar despesas do Firestore: {str(e)}")
        ...
        return Response({
            'status': 'ok',
            'message': 'Despesas carregadas com sucesso (dados estáticos)',
            'despesas': despesas,
            'erro': str(e)
        })

# Adicionar rotas de fallback
urlpatterns += [
    path('categorias-fallback/', categorias_fallback, name='categorias-fallback'),
    path('receitas-fallback/', receitas_fallback, name='receitas-fallback'),
    path('despesas-fallback/', despesas_fallback, name='despesas-fallback'),
] 